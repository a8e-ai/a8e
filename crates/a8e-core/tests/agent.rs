...
mod tests {
    ...
    mod retry_tests {
        use super::*;
        use a8e_core::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use a8e_core::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _model: ModelConfig,
                _extensions: Vec<a8e_core::config::ExtensionConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let a8e_core::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    a8e_core::permission::PermissionConfirmation {
                                        principal_type: a8e_core::permission::permission_confirmation::PrincipalType::Tool,
    ...
    mod extension_manager_tests {
        use super::*;
        use a8e_core::agents::extension::ExtensionConfig;
        use a8e_core::agents::platform_extensions::{
