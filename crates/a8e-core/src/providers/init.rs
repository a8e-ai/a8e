...
pub async fn create(
    ...
    if let Ok(lead_model_name) = config.get_param::<String>("A8E_LEAD_MODEL") {
...
fn create_worker_model_config(
    ...
    if let Ok(limit) = global_config.get_param::<usize>("A8E_WORKER_CONTEXT_LIMIT") {
        worker_config = worker_config.with_context_limit(Some(limit));
    } else if let Ok(limit) = global_config.get_param::<usize>("A8E_CONTEXT_LIMIT") {
        worker_config = worker_config.with_context_limit(Some(limit));
    }
