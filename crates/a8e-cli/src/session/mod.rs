...
use a8e_core::permission::Permission;
...
use a8e_core::conversation::message::{ActionRequiredData, Message, MessageContent};
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: a8e_core::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            a8e_core::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    pub async fn get_session(&self) -> Result<a8e_core::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let model_config =
        ModelConfig::new_with_context_env(model, &provider, Some("A8E_PLANNER_CONTEXT_LIMIT"))?;
    let extensions = a8e_core::config::extensions::get_enabled_extensions_with_config(config);
...
mod tests {
    use super::*;
    use a8e_core::agents::extension::Envs;
