...
fn setup_logging_internal(name: Option<&str>, force: bool) -> Result<()> {
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = a8e_core::logging::prepare_log_directory("cli", true)?;
            ...
            let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                ...
                EnvFilter::new("")
                    ...
                    // Set a8e module to DEBUG
                    .add_directive("a8e=debug".parse().unwrap())
                    // Set a8e-cli to INFO
                    .add_directive("a8e=info".parse().unwrap())
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = a8e_core::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        // Verify directory structure
        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "a8e"));
