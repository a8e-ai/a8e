...
alias @a8e='{a8e_bin} term run'
alias @g='{a8e_bin} term run'

a8e_preexec() {
    [[ "$1" =~ ^a8e\ term ]] && return
    [[ "$1" =~ ^(@a8e|@g)($|[[:space:]]) ]] && return
    ('{a8e_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$a8e_preexec_installed" ]]; then
    a8e_preexec_installed=1
    trap 'a8e_preexec "$BASH_COMMAND"' DEBUG
...
add-zsh-hook preexec a8e_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @a8e; {a8e_bin} term run $argv; end
function @g; {a8e_bin} term run $argv; end

function a8e_preexec --on-event fish_preexec
    string match -q -r '^a8e term' -- $argv[1]; and return
    string match -q -r '^(@a8e|@g)($|\s)' -- $argv[1]; and return
    {a8e_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    ...
    if ($line -notmatch '^a8e term' -and $line -notmatch '^(@a8e|@g)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{a8e_bin}' term log $using:line }} | Out-Null
    }}
...
pub async fn handle_term_init(
    ...
    let a8e_bin = std::env::current_exe()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_else(|_| "a8e".to_string());

    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|s| s.replace("{a8e_bin}", &a8e_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!("AGENT_SESSION_ID not set. Run 'eval \"$(a8e term init <shell>)\"' first.")
    })?;
...
/// Handle `a8e term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = config
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short.strip_prefix("a8e-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = config
        ...
        .ok()
        .and_then(|model_name| {
            config.get_a8e_provider().ok().and_then(|provider_name| {
                a8e_core::model::ModelConfig::new(&model_name)
                    .ok()
