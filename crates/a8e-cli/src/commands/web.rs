...
async fn auth_middleware(
    ...
    *response.status_mut() = StatusCode::UNAUTHORIZED;
    response.headers_mut().insert(
        "WWW-Authenticate",
        "Basic realm=\"Articulate Web Interface\"".parse().unwrap(),
    );
    Ok(response)
}
...
fn get_provider_and_model() -> (String, String) {
    ...
    let provider_name: String = match config.get_a8e_provider() {
        Ok(p) => p,
        Err(_) => {
            eprintln!("No provider configured. Run 'a8e configure' first");
            std::process::exit(1);
        }
    };

    let model: String = match config.get_a8e_model() {
        Ok(m) => m,
        Err(_) => {
            eprintln!("No model configured. Run 'a8e configure' first");
            std::process::exit(1);
        }
    };
...
pub async fn handle_web(
    ...
    crate::logging::setup_logging(Some("a8e-web"))?;
...
async fn serve_session(
    ...
    let html_with_session = html.replace(
        ...
        &format!(
            "<script>window.A8E_SESSION_NAME = '{}'; window.A8E_WS_TOKEN = '{}';</script>\n    <script src=\"/static/script.js\"></script>",
            session_name,
            state.ws_token
        )
    );
...
async fn process_message_streaming(
    ...
    if provider.is_err() {
        let error_msg = "I'm not properly configured yet. Please configure a provider through the CLI first using `a8e configure`.".to_string();
        let mut sender = sender.lock().await;
        let _ = sender
            .send(Message::Text(
