...
use a8e_core::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use a8e_core::providers::base::ConfigKey;
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve Articulate").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve Articulate by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style(
    ...
    println!("{}", style("  • a8e version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'a8e configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve Articulate?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve Articulate!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    println!();
    cliclack::intro(style(" a8e-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "a8e settings",
            "Set the a8e mode, Tool Output, Tool Permissions, Experiment, a8e recipe github repo and more",
        )
        .interact()?;
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &a8e_core::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with A8E_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_a8e_provider().ok();
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                std::env::var("A8E_MODEL").unwrap_or(provider_meta.default_model.clone());
    ...
    let toolshim_enabled = std::env::var("A8E_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("A8E_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(provider_name, &model, toolshim_enabled, toolshim_model).await
    {
        Ok(()) => {
            config.set_a8e_provider(provider_name)?;
            config.set_a8e_model(&model)?;
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&a8e_core::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with a8e",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    let setting_type = cliclack::select("What setting would you like to configure?")
        ...
        .item(
            "recipe",
            "a8e recipe github repo",
            "a8e will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_a8e_mode_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("A8E_MODE").is_ok() {
        let _ = cliclack::log::info("Notice: A8E_MODE environment variable is set and will override the configuration here.");
    }
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("A8E_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info("Notice: A8E_TELEMETRY_OFF environment variable is set and will override the configuration here.");
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve Articulate!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("A8E_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info("Notice: A8E_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.");
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("A8E_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("A8E_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("A8E_CLI_MIN_PRIORITY", 0.0)?;
...
/// Configure experiment features that can be used with a8e
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("A8E_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("A8E_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - a8e will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use a8e_core::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let configured_model: String = config.get_a8e_model()?;
    let model_config = match a8e_core::model::ModelConfig::new(&configured_model) {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use a8e.")?;
                }
                Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use a8e.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
...
fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = a8e_core::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        a8e_core::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
