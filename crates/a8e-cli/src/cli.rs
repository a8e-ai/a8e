...
use a8e_mcp::mcp_server_runner::{serve, McpCommand};
use a8e_mcp::{
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, a8e must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), a8e_core::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with a8e by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to a8e directly",
        long_help = "Input text containing commands for a8e. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=a8e-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the A8E_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the A8E_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum RecipeCommand {
    ...
    /// Open a recipe in Articulate Desktop
    #[command(about = "Open a recipe in Articulate Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Configure a8e settings
    #[command(about = "Configure a8e settings")]
    Configure {},

    /// Display a8e configuration information
    #[command(about = "Display a8e information")]
    Info {
    ...
    #[command(about = "Run one of the mcp servers bundled with a8e")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run a8e as an ACP (Agent Client Protocol) agent
    #[command(about = "Run a8e as an ACP agent server on stdio")]
    Acp {
    ...
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the a8e CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,
    ...
    #[command(
        about = "Terminal-integrated a8e session",
        long_about = "Runs an a8e session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(a8e term init zsh)\"  # Add to ~/.zshrc\n\n\
                      Usage:\n  \
                        a8e term run \"list files in this directory\"\n  \
                        @a8e \"create a python script\"  # using alias\n  \
    ...
    Completion {
        ...
        #[arg(long, default_value = "a8e", help = "Provide a custom binary name")]
        bin_name: String,
    },
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent a8e session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(a8e term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                      With --default (anything typed that isn't a command goes to a8e):\n  \
                        echo 'eval \"$(a8e term init zsh --default)\"' >> ~/.zshrc"
    )]
    Init {
        ...
        /// Make a8e the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make a8e the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to a8e. Only supported for zsh and bash."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        a8e term run list files in this directory\n  \
                        @a8e list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to a8e (multiple words allowed without quotes)
...
async fn handle_mcp_command(server: McpCommand) -> Result<()> {
    ...
    match server {
        ...
        McpCommand::Loop => serve(a8e_mcp::LoopServer::new()).await?,
...
async fn handle_interactive_session(
    ...
    tracing::info!(
        monotonic_counter.a8e.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.a8e.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.a8e.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.a8e.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use a8e run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    a8e_core::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.a8e.recipe_runs = 1,
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(a8e_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    tracing::info!(
        monotonic_counter.a8e.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::Acp { builtins }) => a8e_acp::server::run(builtins).await,
        ...
        Some(Command::ValidateExtensions { file }) => {
            use a8e_core::agents::validate_extensions::validate_bundled_extensions;
